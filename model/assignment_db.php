...
function get_assignments_by_course($course_id){
    ...
    if($course_id){
        ...
        $query = 'SELECT A.ID, A.Description, C.courseName FROM assignments A LEFT JOIN courses C on A.courseID = C.courseID WHERE A.courseID = :course_id ORDER BY C.courseID';
    }
    $statement = $db->prepare($query);
    $statement->bindValue(':course_id', $course_id); // placeholders in the SQL are where actual values will be substituted when the query is executed
    $statement->execute();
